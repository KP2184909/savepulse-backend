...
function businessContent(locale) {
  ...
  const metrics = thai
    ? [["ยอดรวมใบแจ้งหนี้ที่ติดตาม", "USD 148,250", "อ้างอิงเรทล่าสุด 36.72"], ["วันครบกำหนดใกล้สุด", "10 มิ.ย. 2569", "อีก 8 วัน"], ["ความผันผวนค่าเงิน", "ผันผวนต่ำ", "7 วัน +0.52%"]]
    : [["Tracked invoice total", "USD 148,250", "Latest reference 36.72"], ["Nearest due date", "Jun 10, 2026", "8 days left"], ["Currency movement", "Low", "7 days +0.52%"]];
  ...
  const invoices = thai
    ? [["ABC Components Ltd.", "USD 68,250.00 • 31 พ.ค. 2569", "ต้นทุนลดลงโดยประมาณ 1,120.45 บาท", "#0a8d4c", "เทียบกับเรทอ้างอิงก่อนหน้า"], ["Global Packaging Inc.", "EUR 42,130.00 • 6 มิ.ย. 2569", "ต้นทุนเพิ่มขึ้นโดยประมาณ 1,874.32 บาท", "#df3c3c", "ควรติดตามใกล้ชิด"], ["Oceanic Materials Co.", "USD 37,870.00 • 15 มิ.ย. 2569", "ต้นทุนลดลงโดยประมาณ 612.18 บาท", "#0a8d4c", "ผลกระทบยังอยู่ในระดับต่ำ"]]
    : [["ABC Components Ltd.", "USD 68,250.00 • May 31, 2026", "Estimated cost decrease 1,120.45 THB", "#0a8d4c", "Compared with the prior reference rate"], ["Global Packaging Inc.", "EUR 42,130.00 • Jun 6, 2026", "Estimated cost increase 1,874.32 THB", "#df3c3c", "Monitor closely"], ["Oceanic Materials Co.", "USD 37,870.00 • Jun 15, 2026", "Estimated cost decrease 612.18 THB", "#0a8d4c", "Impact remains low"]];
  ...
  return `<tr><td style="padding:0 13px 11px;"><table role="presentation" width="100%"><tr>${metricHtml}</tr></table></td></tr><tr><td style="padding:0 17px 11px;"><table role="presentation" width="100%" style="width:100%;background:#e9fbf7;border-radius:14px;"><tr><td style="width:54px;padding:14px 0 14px 15px;"><div style="width:44px;height:44px;border-radius:50%;background:#0a8e88;color:#fff;text-align:center;line-height:44px;font-size:21px;">◇</div></td><td style="padding:13px 10px;"><div style="color:#0b6e68;font-size:14px;font-weight:900;">${thai ? "สถานะต้นทุน: รอก่อน" : "Cost status: Monitor"}</div><div style="margin-top:3px;color:#406b6b;font-size:9px;line-height:1.45;">${thai ? "ค่าเงินยังผันผวนในกรอบแคบ แนวโน้ม 7 วันยังไม่เพิ่มแรงกดดันต่อต้นทุน" : "Currency movement remains narrow without strong seven-day cost pressure."}</div></td><td align="right" style="padding:13px 15px;color:#0bbcb2;font-size:16px;">▁▃▂▄▆</td></tr></table></td></tr><tr><td style="padding:0 17px 7px;color:#dff5f2;font-size:13px;font-weight:900;">▤ ${thai ? "ใบแจ้งหนี้ที่น่าติดตาม" : "Invoices to monitor"}</td></tr><tr><td style="padding:0 17px 11px;"><table role="presentation" width="100%" style="width:100%;background:#fff;border-radius:13px;overflow:hidden;">${invoiceHtml}</table></td></tr>`;
}
